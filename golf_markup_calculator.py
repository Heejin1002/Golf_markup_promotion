...
def search_tab_ui():
    ...
    if sel_mode == "트리플 골프":
        ...
        with c_coef:
            triple_coef = st.number_input(
            "최소 마진 계수",
            min_value=0.01, max_value=1.0, value=0.89, step=0.01, format="%.2f",
            key="stab_coef",
            help="최소 마진이 보장되는 트리플 판매가(฿)를 역산할 때 사용하는 조정 계수입니다.  \n계산식: 최소 마진 세일(฿) = 패키지넷 ÷ (계수 × (1 - 수수료율))  \n  \n【계수 설정 배경】  \n• 몽키트래블은 현찰살때 환율 기준으로 공급가를 책정하지만, 트리플은 고객 구매 시점의 매매기준율로 정산됩니다 (5분마다 변동).  \n• 두 환율의 차이로 인한 손실을 방지하기 위해 계수로 안전 여유분을 확보합니다.  \n  \n【0.89 설정 이유】  \n0.90 이상은 마진 마이너스, 0.88 이하는 예약 경쟁력 저하 → 0.89가 균형점  \n  \n환율이 크게 변동한 경우 계수를 재검토하세요. (GPT 활용 권장)",
        )
        with c_texr:
